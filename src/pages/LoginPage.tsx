...
import { createWalletSession, findSessionByWallet, getStoredSession } from '../lib/usernameStore';
...
function getSafeNextRoute() {
  ...
  const nextRoute = new URLSearchParams(query).get('next') || getAppHomeRoute();
  const allowedRoutes = ['app/dashboard', 'app/protect', 'app/portfolio', 'app/positions', 'app/claims', 'app/pay-username', 'app/ai-chat', 'app/profile'];
...
export default function LoginPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black p-4 text-[#E1E0CC] md:p-6">
      ...
      <a href="/" className="coverfi-nav-link absolute left-6 top-6 z-20 inline-flex items-center gap-2 text-sm text-[#E1E0CC]/75 transition-colors hover:text-[#E1E0CC]">
      ...
      <section className="relative z-10 flex min-h-[calc(100vh-2rem)] items-center justify-center md:min-h-[calc(100vh-3rem)]">
        ...
        >
          ...
          <h1 className="font-serif text-5xl italic leading-none text-[#E1E0CC] md:text-7xl">Enter CoverFi.</h1>
          <p className="mt-5 text-sm leading-relaxed text-[#E1E0CC]/60">
            Connect with Freighter to open your dashboard. You can claim a unique username later from Profile.
          </p>
