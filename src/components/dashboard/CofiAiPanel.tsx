...
function compactPublicIds(value: string) {
...
function Inline({ value }: { value: string }) {
  return <>{value.split(/(`[^`]+`|\*\*[^*]+\*\*)/g).filter(Boolean).map((part, index) => part.startsWith("`") ? <code key={index} className="break-all rounded bg-white/10 px-1 py-0.5 text-[.9em]">{compactPublicIds(part.slice(1, -1))}</code> : part.startsWith("**") ? <strong key={index}>{compactPublicIds(part.slice(2, -2))}</strong> : <span key={index}>{compactPublicIds(part)}</span>)}</>;
}
...
export default function CofiAiPanel({ open, onClose, username, walletAddress }: { open: boolean; onClose: () => void; username: string; walletAddress: string }) {
  ...
  return <>
    ...
    <aside className="fixed inset-y-0 right-0 z-[61] flex w-full max-w-[440px] flex-col border-l border-violet-200/20 bg-[#0a090e]/95 shadow-[-28px_0_80px_rgba(0,0,0,.5)] backdrop-blur-2xl" aria-label="CoFi AI customer support">
      ...
      <div className="cofi-scroll min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5">{messages.map((item, index) => <div key={index} className={`min-w-0 ${item.role === "user" ? "flex justify-end" : "flex justify-start"}`}><div className={`min-w-0 max-w-[88%] break-words rounded-2xl px-4 py-3 text-sm [overflow-wrap:anywhere] ${item.role === "user" ? "bg-[#E1E0CC] text-black" : "border border-[#E1E0CC]/10 bg-white/[.04] text-[#E1E0CC]/75"}`}>{item.role === "assistant" ? <Markdown text={item.text} /> : compactPublicIds(item.text)}</div></div>)}{loading && <div className="flex items-center gap-2 text-xs text-[#E1E0CC]/50"><img src="/logo.png" alt="CoverFi" className="h-5 w-5 animate-pulse object-contain" />{mode === "research" ? "Researching approved CoverFi sources…" : "Preparing final answer…"}</div>}<div ref={messageEndRef} /></div>
